...
import { Text, View, TouchableOpacity, Image, StyleSheet, Modal, Alert } from 'react-native';
...
const PlayPageModal = ({ visible}) => {
  ...
  const handleDownButtonPress = () => {
      setIsExpanded(false)
      console.log("down" + isExpanded);
      Alert("down" + isExpanded)
  };
// 
...
const restartSong = async () => {
  ...
  return (
      ...
      <Modal visible={isExpanded} transparent animationType='slide'>
        ...
        <TouchableOpacity style = {{height:50, width: 50}} onPress={handleDownButtonPress}>
